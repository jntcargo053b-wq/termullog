...
class LayoutDSLRCorner extends WatermarkLayoutBase {
  ...
  static const bool _positionBottom = true; // true = bottom, false = top (untuk corner, bottom right)
  ...
  Uint8List apply({
    ...
    final int lHL = isLargeFont ? (38 * scale).round() : isSmallFont ? (20 * scale).round() : (28 * scale).round();
    final int lH  = isLargeFont ? (28 * scale).round() : isSmallFont ? (16 * scale).round() : (20 * scale).round();
    ...
    if ((showCoordinates || showAccuracy) && hasPosition &&
        (showAddress && _cleanAddr(address).isNotEmpty || (showWeather && weather.isNotEmpty))) rowCount += 1;
    final cleanAddr = _cleanAddr(address);
    if (showAddress && cleanAddr.isNotEmpty) rowCount += _splitAddr(cleanAddr, (src.width * 0.25).toInt()).length;
    ...
    final int innerH = lHL + lH + _sepH + sepSpace
        ...
        + ((showCoordinates || showAccuracy) && hasPosition &&
           (showAddress && cleanAddr.isNotEmpty || (showWeather && weather.isNotEmpty)) ? _sepH + sepSpace : 0)
        + (showAddress && cleanAddr.isNotEmpty ? _splitAddr(cleanAddr, (src.width * 0.25).toInt()).length * lH : 0)
        + (showWeather && weather.isNotEmpty ? lH : 0);

    final int cardH = _accentH + padY * 2 + innerH + 6;
    final int cardW = isLargeFont ? (src.width * 0.50).clamp(320, 460).toInt()
        : isSmallFont ? (src.width * 0.30).clamp(180, 280).toInt()
    ...
    img.fillCircle(src, x: recX - 8, y: ty + 9, radius: 4, color: img.ColorRgba8(255, 50, 50, 255));
    ty += lHL;
    ...
    _shadow(src, dayStr, font: fontS, x: cx + cardW - dayStr.length * 7 - padX, y: ty, color: _dim);
    ty += lH;
  ...
  void _shadow(img.Image src, String text, {required img.BitmapFont font, required int x, required int y, required img.Color color}) {
