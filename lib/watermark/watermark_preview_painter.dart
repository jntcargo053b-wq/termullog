...
class WatermarkPreviewPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double currentFontScale = fontScale.clamp(0.5, 2.0);

    _drawReferenceLayout(canvas, W, H, sc, currentFontScale);
  }
  ...
  void _drawReferenceLayout(Canvas c, double W, double H, double sc, double fontScale) {
    ...
    // 1. Measure Time
    final String timeStr = DateFormat(timeFormat.isNotEmpty ? timeFormat : 'HH:mm:ss').format(timestamp);
    ...
    double mapW = 0;
    if (showMiniMap) {
      mapW = mapSize.toDouble() * sc;
    }

    final double basePanelW = (headerRowW + panelPaddingHorizontal * 2).clamp(700 * sc, W - 64 * sc);
    final double panelW = showMiniMap ? max(basePanelW, mapW + panelPaddingHorizontal * 2) : basePanelW;
    ...
    // --- Mini Map Section ---
    if (showMiniMap && mapImage != null) {
      final double mapH = mapW; 
      c.drawImageRect(
        mapImage!,
  ...
  double _calculatePanelHeight(double sc, double fontScale, double panelW, double verticalPadding, double contentGap, double dividerPadding) {
    ...
    if (showMiniMap) {
      h += (mapSize.toDouble() * sc) + contentGap;
    }
  ...
  bool shouldRepaint(WatermarkPreviewPainter oldDelegate) {
    return oldDelegate.timestamp != timestamp ||
        ...
        oldDelegate.showMiniMap != showMiniMap ||
