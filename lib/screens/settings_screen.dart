...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Widget _buildPreview() {
    ...
    String dateStr = '15/05/2024', timeStr = '14:30:25';
    if (_dateFormat == 'yyyy-MM-dd') dateStr = '2024-05-15';
    else if (_dateFormat == 'dd MMM yyyy') dateStr = '15 Mei 2024';
    else if (_dateFormat == 'MMMM dd, yyyy') dateStr = 'Mei 15, 2024';
    if (_timeFormat == 'HH:mm') timeStr = '14:30';
    else if (_timeFormat == 'hh:mm:ss a') timeStr = '02:30:25 PM';

    switch (_selectedLayout) {
      case WatermarkLayout.minimal:
        return Container(padding: const EdgeInsets.all(12), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [Container(height: 3, width: 40, color: const Color(0xFF00B8D4)), const SizedBox(height: 8), Text('TERMULOG', style: TextStyle(color: textColor, fontSize: 12, fontWeight: FontWeight.bold)), const SizedBox(height: 4), Text('$dateStr  $timeStr', style: const TextStyle(color: Colors.white70, fontSize: 9)), if (_showCoordinates) Text('-6.123456, 106.123456', style: const TextStyle(color: Colors.white70, fontSize: 9)), if (_showAccuracy) Text('±5 m', style: TextStyle(color: Colors.grey.shade500, fontSize: 9)), if (_showAddress) Text('Jl. Contoh No. 123', style: TextStyle(color: Colors.grey.shade500, fontSize: 8))]));
      case WatermarkLayout.modern:
        return Container(padding: const EdgeInsets.all(12), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [Container(width: 3, height: 20, color: const Color(0xFF00B8D4)), const SizedBox(width: 8), Text('📍 TERMULOG', style: TextStyle(color: textColor, fontSize: 12, fontWeight: FontWeight.bold))]), const SizedBox(height: 8), Text('$timeStr  •  $dateStr', style: const TextStyle(color: Colors.white70, fontSize: 9)), if (_showCoordinates) Text('🌐 -6.123456°, 106.123456°', style: const TextStyle(color: Colors.white70, fontSize: 9)), if (_showAccuracy) Text('🎯 Akurasi: ±5 m', style: TextStyle(color: textColor, fontSize: 9)), if (_showWeather) Text('🌤️ Cerah 32°C', style: TextStyle(color: textColor, fontSize: 9))]));
      case WatermarkLayout.elegant:
        return Container(padding: const EdgeInsets.all(12), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [Text('TERMULOG', style: TextStyle(color: const Color(0xFFFFB432), fontSize: 12, fontWeight: FontWeight.bold)), const SizedBox(height: 8), Text(dateStr, style: const TextStyle(color: Colors.white70, fontSize: 9)), Text(timeStr, style: TextStyle(color: textColor, fontSize: 9)), if (_showCoordinates) Text('-6.123456°\n106.123456°', style: const TextStyle(color: Colors.white70, fontSize: 9)), if (_showAccuracy) Text('Akurasi ±5 m', style: TextStyle(color: Colors.grey.shade500, fontSize: 9))]));
      case WatermarkLayout.professional:
        return Container(padding: const EdgeInsets.all(12), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [Container(width: double.infinity, color: const Color(0xFF00B8D4), padding: const EdgeInsets.symmetric(vertical: 4, horizontal: 8), child: const Text('TERMULOG DOCUMENT', style: TextStyle(color: Colors.black, fontSize: 9, fontWeight: FontWeight.bold))), const SizedBox(height: 8), Text('DATE : $dateStr', style: const TextStyle(color: Colors.white70, fontSize: 8)), Text('TIME : $timeStr', style: const TextStyle(color: Colors.white70, fontSize: 8)), if (_showCoordinates) Text('LAT : -6.123456\nLON : 106.123456', style: TextStyle(color: Colors.grey.shade500, fontSize: 8)), if (_showAccuracy) Text('ACC : ±5 m', style: TextStyle(color: Colors.grey.shade500, fontSize: 8)), if (_showAddress) Text('ADDR : Jl. Contoh No. 123', style: TextStyle(color: Colors.grey.shade500, fontSize: 7))]));
      case WatermarkLayout.semiTransparent:
        return Container(padding: const EdgeInsets.all(12), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [Text('$dateStr   $timeStr', style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w500)), const SizedBox(height: 8), if (_showCoordinates) Text('-6.123456, 106.123456', style: TextStyle(color: textColor, fontSize: 11, fontWeight: FontWeight.w500)), if (_showAccuracy) Text('±5 m', style: TextStyle(color: Colors.grey.shade500, fontSize: 10))]));
      case WatermarkLayout.gpsCard:
        return Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.black, border: Border(bottom: BorderSide(color: const Color(0xFF00B8D4), width: 3))), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [Text('$dateStr  $timeStr', style: const TextStyle(color: Colors.white, fontSize: 10)), const SizedBox(height: 6), Text('-6.123456°N  106.123456°E', style: TextStyle(color: const Color(0xFF00B8D4), fontSize: 10)), if (_showAccuracy) Text('±5 m', style: TextStyle(color: Colors.grey.shade500, fontSize: 9)), const SizedBox(height: 4), Text('Jl. Contoh No. 123, Kec. Contoh', style: TextStyle(color: Colors.grey.shade500, fontSize: 8), maxLines: 2)]));
      case WatermarkLayout.polaroid:
        return Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: const Color(0xFFF8F5EB), border: Border.all(color: Colors.grey.shade300)), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [Container(height: 80, width: double.infinity, color: Colors.grey.shade400, child: const Center(child: Icon(Icons.image, color: Colors.grey, size: 32))), const SizedBox(height: 8), Text(dateStr, style: const TextStyle(color: Colors.black87, fontSize: 10)), Text('-6.123, 106.123', style: const TextStyle(color: Colors.black54, fontSize: 9))]));
      case WatermarkLayout.sidePanel:
        return Row(children: [Container(width: 50, height: 120, color: const Color(0xFF0A0F28), padding: const EdgeInsets.symmetric(vertical: 8, horizontal: 4), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Text('14', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)), Text('30', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)), Text('25', style: TextStyle(color: const Color(0xFF00B8D4), fontSize: 18, fontWeight: FontWeight.bold)), const SizedBox(height: 8), Text('15', style: TextStyle(color: Colors.white70, fontSize: 12)), Text('Mei', style: TextStyle(color: const Color(0xFF00B8D4), fontSize: 12))])), const SizedBox(width: 8), Expanded(child: Container(height: 120, color: Colors.grey.shade800, child: const Center(child: Icon(Icons.image, color: Colors.grey, size: 40))))]);
      case WatermarkLayout.cinematicV2:
      case WatermarkLayout.timeMarkStyle:
        return Container(padding: const EdgeInsets.all(12), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [Text(timeStr, style: TextStyle(color: textColor, fontSize: 18, fontWeight: FontWeight.bold)), const SizedBox(height: 4), Text(dateStr, style: const TextStyle(color: Colors.white70, fontSize: 10))]));
    }
  }
  ...
  Color _getPreviewTextColor() {
    switch (_selectedLayout) {
      ...
      case WatermarkLayout.modern: case WatermarkLayout.elegant: case WatermarkLayout.gpsCard: case WatermarkLayout.polaroid: case WatermarkLayout.sidePanel: case WatermarkLayout.cinematicV2: case WatermarkLayout.timeMarkStyle: return const Color(0xFF00B8D4);
      case WatermarkLayout.professional: return Colors.white70;
      case WatermarkLayout.semiTransparent: return const Color(0xFF00B8D4);
    }
  }
