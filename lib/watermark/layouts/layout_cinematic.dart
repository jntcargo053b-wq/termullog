...
class LayoutCinematic extends WatermarkLayoutBase {
  ...
  Uint8List apply({
    ...
    // Fallback: pakai img.arial24 (isolate)
  ...
  // ─── ASYNC FLUTTER CANVAS (main thread) ─────────────────────────
  @override
  Future<Uint8List> applyAsync({
    ...
    final int mapH = (120 * scale).round();
    final int lineH = (32 * scale * fsMultiplier).round();
    final int smallLine = (24 * scale * fsMultiplier).round();
    final int padX = (_padX * scale).round();
    final int padY = (_padY * scale).round();
    final int margin = (12 * scale).round();
    ...
    canvas.drawRRect(cardRect,
        Paint()..color = Colors.white.withOpacity(0.08)..style = PaintingStyle.stroke..strokeWidth = 1);
    ...
    if (showMiniMap && mapBytes != null && mapBytes.isNotEmpty) {
      try {
        ...
        if (map != null) {
          ...
          final mapRect = Rect.fromLTWH(cardX, cy, cardW, mapH.toDouble());
          // Overlay gelap
    ...
    WatermarkLayoutBase.canvasDrawTextShadow(canvas,
        ...
        x: cardX + padX, y: cy, color: Colors.white, bold: false, size: 16 * fsMultiplier);
    cy += lineH;

    // Jam
    WatermarkLayoutBase.canvasDrawTextShadow(canvas,
        DateFormat('HH:mm:ss').format(timestamp),
        x: cardX + padX, y: cy, color: Colors.white, bold: true, size: 22 * fsMultiplier);
    cy += lineH;

    // Divider
    canvas.drawLine(
        Offset(cardX + padX, cy),
        Offset(cardX + cardW - padX, cy),
    ...
    if (showAddress && address.isNotEmpty) {
      ...
      for (final line in lines.take(2)) {
        WatermarkLayoutBase.canvasDrawTextShadow(canvas, line,
            x: cardX + padX, y: cy, color: Colors.white70, bold: false, size: 13 * fsMultiplier);
        cy += smallLine;
      }
    }
    ...
    if (showCoordinates && hasPosition) {
      WatermarkLayoutBase.canvasDrawTextShadow(canvas,
          ...
          x: cardX + padX, y: cy, color: const Color(0xFF1E90FF), bold: false, size: 13 * fsMultiplier);
    }
