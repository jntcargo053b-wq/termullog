...
class _CameraScreenState extends State<CameraScreen> with WidgetsBindingObserver {
  ...
  // Early address fetch — tampilkan alamat secepat mungkin dari GPS pertama
  ...
  void _warmupGps() async {
    try {
      ...
      if (!_isDisposed && warmup.accuracy <= CameraConstants.accuracyMax && !warmup.isMocked) {
        ...
        // Langsung fetch alamat dari posisi warmup — tidak tunggu sample terkumpul
        if (!_earlyAddressFetched) {
          _earlyAddressFetched = true;
          if (_bestPosition == null) _bestPosition = warmup;
          _fetchAddressEarly(warmup);
        }
      }
    } catch (_) {}
  }
  ...
  // Fetch alamat segera dari posisi GPS pertama (tanpa debounce).
  // Dipanggil sekali dari _warmupGps atau sample GPS pertama.
  // Hasilnya ditampilkan sebagai alamat sementara; akan di-overwrite saat
  // _fetchAddressForCurrentPosition dipanggil dengan posisi yang lebih baik.
  Future<void> _fetchAddressEarly(Position pos) async {
    try {
      ...
      if (addr.isNotEmpty && !addr.startsWith('GPS:')) {
        ...
        // Hanya update UI jika belum ada alamat lebih baik
        if (_currentAddress.isEmpty) {
          setState(() => _currentAddress = addr);
        }
        debugPrint('Early address: $addr');
      }
    } catch (e) {
  ...
  void _onGpsData(Position pos) {
    ...
    // Early fetch: jika warmup belum berhasil mendapat alamat, gunakan
    // sample stream pertama yang valid agar alamat muncul secepat mungkin.
    if (!_earlyAddressFetched) {
      _earlyAddressFetched = true;
  ...
  // ------------------------------------------------------------
  // Overlay realtime di atas kamera (Adaptive - glassmorphism)
  // ------------------------------------------------------------
