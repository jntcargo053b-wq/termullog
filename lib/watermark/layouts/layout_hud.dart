...
class LayoutHUD extends WatermarkLayoutBase {
  ...
  Uint8List apply({
    ...
    // ── Top gradient (no hard border) ────────────────────────────
    for (int y = 0; y < topH + (8 * scale).round(); y++) {
      if (y >= src.height) break;
      final t = 1.0 - (y / (topH + (8 * scale)));
    ...
    // ── Corner brackets ──────────────────────────────────────────
    _bracket(src, x: bracketInset, y: bracketInset, fx: 1, fy: 1, arm: bracketArm, thick: bracketThick);
    _bracket(src, x: src.width - bracketInset, y: bracketInset, fx: -1, fy: 1, arm: bracketArm, thick: bracketThick);
    _bracket(src, x: bracketInset, y: src.height - bracketInset, fx: 1, fy: -1, arm: bracketArm, thick: bracketThick);
    _bracket(src, x: src.width - bracketInset, y: src.height - bracketInset, fx: -1, fy: -1, arm: bracketArm, thick: bracketThick);
    ...
    // ── Bottom gradient (no hard border) ─────────────────────────
    final y0 = src.height - botH;
    for (int y = y0 - (10 * scale).round(); y < src.height; y++) {
      if (y < 0 || y >= src.height) continue;
      final t = (y - (y0 - (10 * scale))) / (botH + (10 * scale));
      final alpha = (t.clamp(0.0, 1.0) * 200).toInt().clamp(0, 200);
      for (int x = 0; x < src.width; x++) {
        final px = src.getPixel(x, y);
        src.setPixel(x, y, img.ColorRgba8(
          ((px.r * (255 - alpha)) ~/ 255),
    ...
    // Accent bar tetap dipertahankan
