...
class LayoutGpsCard extends WatermarkLayoutBase {
  ...
  Uint8List apply({
    ...
    img.fillRect(src, x1: 0, y1: y0, x2: src.width - 1, y2: y0 + panelH,
        color: img.getColor(0, 0, 10, (230 * opacity).toInt()));
    img.fillRect(src, x1: 0, y1: y0, x2: src.width - 1, y2: y0 + 4,
        color: img.getColor(0, 180, 255, 255));
    ...
    img.drawString(src, font, padX + 16, cy, DateFormat('yyyy-MM-dd HH:mm:ss').format(timestamp), color: WatermarkLayoutBase.white);
    cy += lineH;

    if (showCoordinates && hasPosition) {
      img.drawString(src, font, padX + 16, cy, '${lat!.toStringAsFixed(6)}  ${lon!.toStringAsFixed(6)}', color: img.getColor(0, 180, 255, 255));
      cy += lineH;
    }

    if (showAccuracy && hasPosition) {
      img.drawString(src, smallFont, padX + 16, cy, '±${acc?.toStringAsFixed(0) ?? '?'} m', color: WatermarkLayoutBase.grey);
      cy += lineH;
    }
    ...
    if (showAddress && address.isNotEmpty && address != 'Tidak ada lokasi' && !address.startsWith('GPS:')) {
      ...
      img.drawString(src, smallFont, padX + 16, cy, shortAddr, color: WatermarkLayoutBase.grey);
      cy += lineH;
    }

    if (showWeather && weather.isNotEmpty) {
      img.drawString(src, smallFont, padX + 16, cy, weather, color: img.getColor(0, 180, 255, 255));
    }
        ...
        if (map != null) {
          ...
          if (mx >= 0 && my >= 0) {
            img.drawRect(src, x1: mx - 2, y1: my - 2, x2: mx + mapW + 1, y2: my + mapH + 1,
                color: img.getColor(0, 180, 255, 100));
