...
class LayoutNamaBaru extends WatermarkLayoutBase {
  ...
  Uint8List apply({
    ...
    final int textX = _padX + (showBorder ? _accentW + _accentGap : 0);
    ...
    final lineH = fontSize == 'small' ? 20 : fontSize == 'large' ? 30 : _lineH;
    final lineS = fontSize == 'small' ? 16 : fontSize == 'large' ? 24 : _lineS;
  ...
  String _fmtLat(double v) => '${v.abs().toStringAsFixed(5)}° ${v >= 0 ? "N" : "S"}';
  String _fmtLon(double v) => '${v.abs().toStringAsFixed(5)}° ${v >= 0 ? "E" : "W"}';
  ...
  String _cleanAddress(String raw) {
    ...
    if (result.length > 3) result.removeRange(3, result.length);
    return result.join(', ');
  }
